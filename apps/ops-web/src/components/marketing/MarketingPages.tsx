...
function Card({
  ...
  return (
    <article className="rounded-[28px] border border-white/8 bg-white/[0.03] p-6 md:p-8">
      ...
      <p className="mt-4 text-sm leading-7 text-[#B6B6AF] md:text-base">{description}</p>
...
function BulletList({ items }: { items: string[] }) {
  return (
    <ul className="space-y-3">
      {items.map((item) => (
        <li key={item} className="flex gap-3 text-sm leading-7 text-[#D8D8D1] md:text-base">
        ...
        <div className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
          <div className="space-y-8">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <SectionLink href={page.hero.secondary.href} label={page.hero.secondary.label} />
            </div>
          </div>
          ...
          <div className="rounded-[32px] border border-white/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))] p-6 md:p-8">
            ...
            <div className="mt-6 space-y-5">
              {page.hero.trust.map((item, index) => (
                <div key={item} className="rounded-3xl border border-white/8 bg-[#0D0D0C] p-5">
                  <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#7EF3C5]">
                    {String(index + 1).padStart(2, "0")}
                  </p>
                  <p className="mt-3 text-base leading-7 text-[#ECECE6]">{item}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {page.stages.map((stage) => (
            <Card key={stage.title} title={stage.title} description={stage.description}>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-2">
          {page.servicesIntro.cards.map((service) => (
            ...
            >
              <SectionLink href={`/${locale}/services/${service.slug}`} label="Ver detalle" />
            </Card>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24 pt-10 md:px-10 md:pb-32">
        <div className="rounded-[36px] border border-white/8 bg-[linear-gradient(135deg,rgba(126,243,197,0.08),rgba(245,213,139,0.08))] p-8 md:p-12">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <SectionLink href={page.finalCta.secondary.href} label={page.finalCta.secondary.label} />
          </div>
        </div>
      </section>
    </MarketingShell>
  ...
  return (
    <MarketingShell locale={locale}>
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10 md:py-24">
        <SectionIntro eyebrow="Estudio" title="Noctra existe para negocios que ya no quieren improvisar su presencia digital." description={page.intro} />
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
      ...
      <section className="mx-auto max-w-7xl px-6 py-8 md:px-10 md:py-16">
        <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <Card
            ...
            description="Noctra mezcla criterio de marca, estructura web, visibilidad y operación para que el sistema completo sea más sólido."
          >
            <div className="space-y-5">
              {page.differentiators.map((item) => (
                <div key={item.title} className="border-t border-white/10 pt-5 first:border-t-0 first:pt-0">
                  <h3 className="text-lg font-black text-[#F5F5F0]">{item.title}</h3>
                  <p className="mt-2 text-sm leading-7 text-[#B6B6AF]">{item.description}</p>
                </div>
              ))}
            </div>
          </Card>
...
export function ServicesPage({ locale }: { locale: string }) {
  ...
  return (
    <MarketingShell locale={locale}>
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10 md:py-24">
        <SectionIntro eyebrow="Servicios" title="Diseñamos la base digital que una empresa necesita en su etapa real." description={page.intro} />
        <div className="mt-10 grid gap-6 lg:grid-cols-2">
          {page.cards.map((service) => (
            <Card key={service.slug} title={service.title} description={service.description} eyebrow={service.outcome}>
              <SectionLink href={`/${locale}/services/${service.slug}`} label="Ver detalle" />
            </Card>
          ))}
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24 md:px-10 md:pb-32">
        <Card title={page.continuity.title} description={page.continuity.description}>
...
export function ServiceDetailPage({
  ...
  return (
    <MarketingShell locale={locale}>
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10 md:py-24">
        <SectionIntro eyebrow={page.eyebrow} title={page.title} description={page.intro} />
        <div className="mt-10 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <Card title="Para quién aplica" description="Si te ves en varios de estos escenarios, esta ruta probablemente tiene sentido.">
            <BulletList items={page.fit} />
          </Card>
          <Card title="Qué incluimos" description="El alcance final depende del proyecto, pero esta es la lógica base de implementación.">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24 md:px-10 md:pb-32">
        <Card title="Qué debería cambiar después" description="Lo importante no es entregar piezas. Es notar una mejora en comprensión, confianza, visibilidad o flujo operativo.">
...
export function WorkPage({ locale }: { locale: string }) {
  ...
  return (
    <MarketingShell locale={locale}>
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10 md:py-24">
        <SectionIntro eyebrow="Resultados" title="Preferimos mostrar problemas resueltos, no promesas vacías." description={page.intro} />
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {page.cases.map((item) => (
            <Card key={item.name} title={item.name} description={item.challenge} eyebrow={item.sector}>
              <div className="space-y-5">
                <div>
                ...
                <div className="border-t border-white/10 pt-5">
                  ...
                  <p className="mt-3 text-sm leading-7 text-[#D8D8D1]">{item.outcome}</p>
                </div>
              </div>
            </Card>
          ))}
        </div>
        <p className="mt-8 max-w-3xl text-sm leading-7 text-[#8F8F88]">{page.note}</p>
      </section>
    </MarketingShell>
  );
}
    ...
    <MarketingShell locale={locale}>
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10 md:py-24">
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="space-y-8">
            <SectionIntro eyebrow="Contacto" title="Cuéntanos qué necesitas desbloquear." description={page.intro} />
            <Card
...
export function BlogPage({ locale }: { locale: string }) {
  ...
  return (
    <MarketingShell locale={locale}>
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10 md:py-24">
        <SectionIntro eyebrow="Insights" title="Contenido para negocios que quieren criterio, no ruido digital." description={page.intro} />
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {page.posts.map((post) => (
            <Card key={post.title} title={post.title} description={post.summary} eyebrow={post.category} />
          ))}
        </div>
      </section>
    </MarketingShell>
...
export function GuaranteePage({ locale }: { locale: string }) {
  ...
  return (
    <MarketingShell locale={locale}>
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10 md:py-24">
        <SectionIntro eyebrow="Garantía" title="La confianza se construye mejor cuando el riesgo está aterrizado." description={page.intro} />
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {page.principles.map((item) => (
            <Card key={item.title} title={item.title} description={item.description} />
          ))}
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24 md:px-10 md:pb-32">
        <Card title="Condiciones de aplicación" description="La garantía no se presenta como truco comercial; se detalla según el proyecto y la responsabilidad compartida.">
...
export function TechnologyPage({ locale }: { locale: string }) {
  ...
  return (
    <MarketingShell locale={locale}>
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10 md:py-24">
        <SectionIntro eyebrow="Tecnología" title="Explicamos la tecnología en términos de negocio." description={page.intro} />
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
...
export function PricingPage({ locale }: { locale: string }) {
  ...
  return (
    <MarketingShell locale={locale}>
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10 md:py-24">
        <SectionIntro eyebrow="Precios" title="Cotizamos según la etapa del negocio, no desde una tabla ciega." description={page.intro} />
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {page.tiers.map((tier) => (
            <Card key={tier.name} title={tier.name} description={tier.fit} eyebrow={tier.range}>
...
export function FirstClientsPage({ locale }: { locale: string }) {
  ...
  return (
    <MarketingShell locale={locale}>
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10 md:py-24">
        <SectionIntro eyebrow="Programa fundacional" title="Una colaboración temprana, estratégica y limitada." description={page.intro} />
        <div className="mt-10 grid gap-6 lg:grid-cols-2">
          <Card title="Qué obtienes" description="Cuando abrimos estos espacios, el nivel de cercanía y acompañamiento es mayor que en un proyecto estándar.">
            <BulletList items={page.benefits} />
          </Card>
          <Card title="Cómo evaluamos fit" description="No es una promoción abierta para cualquiera. Seleccionamos por claridad del reto y valor estratégico mutuo.">
            <BulletList items={page.conditions} />
            <div className="mt-8">
              <SectionLink href={page.cta.href} label={page.cta.label} />
            </div>
...
export function CareersPage({ locale }: { locale: string }) {
  ...
  return (
    <MarketingShell locale={locale}>
      <section className="mx-auto max-w-7xl px-6 py-16 md:px-10 md:py-24">
        <SectionIntro eyebrow="Talento" title="Trabajamos con una red pequeña de especialistas con criterio." description={page.intro} />
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {page.roles.map((role) => (
