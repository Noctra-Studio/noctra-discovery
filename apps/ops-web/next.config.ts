...
// Keep local development working while the repo transitions into a monorepo.
loadEnv({ path: path.resolve(currentDir, ".env.local") });
loadEnv({ path: path.resolve(currentDir, "../../.env.local"), override: false });
...
const nextConfig = {
  ...
  transpilePackages: ["@noctra-ops/db-types", "@noctra-ops/validations"],
  async headers() {
    return [
      {
        source: "/(.*)",
